...
class DownBlock(nn.Module):
    def __init__(self, in_channels=None, nFeat=None, out_channels=64):
        super(DownBlock, self).__init__()

        dual_block = [nn.Sequential(
            nn.Conv2d(in_channels, nFeat, kernel_size=5, stride=5, padding=0, bias=False),
...
class Upsampler(torch.nn.Module):
    def __init__(self, n_feats):
        ...
        self.conv = nn.Conv2d(n_feats, 4 * n_feats, 3, stride=1, padding=1)
